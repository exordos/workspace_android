...
| AUTH-002 | Reject invalid or unsafe server discovery | Invalid/non-origin URL, cross-origin redirect, DNS/TLS failure, timeout, 404, permission, 429/5xx, malformed/oversized JSON, unsafe realm/meeting/icon metadata | A content-free actionable category is shown and no pending organization is saved; the final response retains the entered origin and only bounded credential-free HTTPS public metadata reaches login | URL/settings/error/origin policy unit + MockEngine same/cross-origin redirect, status, malformed/oversized body, SSL/CIO and Android certificate failures + Android instrumented + physical DNS and expired-certificate failures passed |
| AUTH-003 | Login by nickname | Valid credentials | Tokens stored encrypted; main navigation opens | Android real-client MockEngine token/project flow and live primary-account nickname -> OTP -> project -> messenger navigation passed on a physical Android 14 device |
| AUTH-004 | Login by email | Valid credentials | Same account identity as server response | Android real-client MockEngine token/project flow passed; live backend identity acceptance remains |
| AUTH-005 | Invalid credentials | Wrong password | Field-safe error; password not logged or persisted | Android real-client MockEngine 401 path and saved-state secret exclusion passed; live backend remains |
| AUTH-006 | OTP challenge | Correct six-digit code | One continuation request; authenticated session | Android real-client MockEngine challenge/header/project-token path and live primary-account OTP continuation passed on a physical Android 14 device |
| AUTH-007 | OTP failure and retry | Wrong, expired, then valid code | Credentials retained safely; no duplicate session | Android real-client MockEngine wrong-then-valid retry passed with one final session; expired-code/live backend remains |
| AUTH-008 | Rotate token during request burst | Concurrent 401 responses plus timeout/network/429/5xx token-endpoint failures | Exactly one refresh runs at a time; timeout/network/429/5xx failures get two automatic retries through the owner-scoped monotonic backoff before later triggers continue the 1s/2s/4s/8s/16s/30s schedule, success resets it, and another owner is never delayed | Unit + Android real-client MockEngine matrix passed for 429/503/network/malformed/identity mismatch, same-call 503-to-success rotation and four concurrent refreshes |
| AUTH-009 | Refresh rejected | OAuth `invalid_grant` / backend invalid-refresh-token response, generic 401 or 403, and a second 401 after successful rotation | Only the explicit invalid-refresh code removes the rejected active owner; generic status failures and protected-resource rejection preserve the rotated credentials and return a recoverable error | Unit + owner/storage-failure instrumentation + Android real-client 400/401/403 and post-rotation 401 matrix passed; a generated invalid refresh token was rejected by the live endpoint and removed only its isolated test owner |
...
| AUTH-011 | Logout current organization | Push registered, cached messages, retained draft/outbox; injected attachment-cache or encrypted-credential write failure | Push registration is removed; encrypted conversation state, private attachment cache and both credentials for only that account are cleared before the registry entry; storage failure retains the active owner, restores the credential pair and permits an explicit retry | Remote push E2E + injected Android storage-failure instrumentation |
| AUTH-012 | Process death during credentials/in-flight login/OTP/project selection | Background process kill after saved-state capture | Bounded nickname/email returns; password, OTP, tokens and project data do not; every secret-bearing intermediate phase fails closed to credentials with an explicit re-entry message; successful login removes all retained login metadata | Saved-state unit matrix, Android real-client OTP/project recreation, and physical credential, live OTP and live project-step process death passed on Android 14; each intermediate step restored only the bounded login and explicit re-entry message |
