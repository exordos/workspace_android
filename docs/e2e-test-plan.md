...
| AUTH-008 | Rotate token during request burst | Concurrent 401 responses plus timeout/network/429/5xx token-endpoint failures | Exactly one refresh runs at a time; later triggers wait outside the mutex on an owner-scoped monotonic 1s/2s/4s/8s/16s/30s backoff, success resets the schedule, and another owner is never delayed | Unit + Android real-client MockEngine matrix passed for 429/503/network/malformed/identity mismatch, 503-to-success rotation and four concurrent refreshes |
| AUTH-009 | Refresh rejected | OAuth `invalid_grant`, backend `InvalidRefreshTokenError`, 401 or 403 | No retry/backoff is scheduled; account-scoped drafts/outbox and private caches are cleared before only the rejected active owner is removed; all work stops and login is shown once | Unit + owner/storage-failure instrumentation + Android 400/401/403 real-client matrix passed; a generated invalid refresh token was rejected by the live endpoint and removed only its isolated test owner |
