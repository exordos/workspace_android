...
| Realtime | REST catch-up, websocket, epoch cursor reset | Partial: one retained repository now owns a foreground-scoped socket; backgrounding cancels it, foregrounding resumes from the in-memory cursor, flapping connections use bounded exponential backoff, and close `4410` clears the expired cursor/derived projections while retaining local outbox rows and forcing the catalog/open history to reload | Add durable account-scoped cursor persistence, explicit REST catch-up before live delivery, a silent-socket epoch watchdog, visible connection health and controlled lifecycle/410/4410 E2E |
