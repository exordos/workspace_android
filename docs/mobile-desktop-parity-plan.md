...
| Message sending | Text, selection-aware Markdown formatting, local preview, optimistic row, retry/remove failed send | Partial: all ten maintained desktop formatting actions now mutate the exact selection/cursor with localized 48 dp controls and authoritative size-bound reconciliation; Write/Preview renders the exact plain or ordered multi-reply outgoing body through the production message renderer without a server mutation; an encrypted account/conversation-scoped outbox is persisted before POST; optimistic rows survive Activity/process recreation and a fully cold offline exact-topic start; rejected and ambiguous outcomes expose different retry/verify/remove paths; successful responses are validated against the exact conversation and payload | Complete physical edit/existing-URN attachment preview, IME/composition and controlled process-death/account-switch checks; expand real-backend/process-failure E2E to attachments, edits and concurrent sends; backend request idempotency is required before ambiguous delivery can be retried automatically without duplicate risk |
