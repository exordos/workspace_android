...
| Session lifecycle | Refresh token, logout, expired-session recovery | Partial: refresh is single-flight and owner-bound; transient token-endpoint failures now keep the account and gate later refresh triggers behind an owner-scoped 1s/2s/4s/8s/16s/30s monotonic backoff outside the shared mutex, while OAuth `invalid_grant`, the backend `InvalidRefreshTokenError`, 401 and 403 remove only the still-active rejected owner through the serialized cleanup path; successful rotation resets that owner's schedule; the account-scoped 6h/12h/24h/3d/7d/Never inactivity policy persists a bounded checkpoint across process death | Finish push-cleanup/storage-failure and live token-endpoint fault E2E |
