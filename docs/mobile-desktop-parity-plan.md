...
| Rich content | Markdown, mentions, Workspace references, links | Partial: canonical `urn:url:http(s)://...` links now unwrap to the exact safe external target with query/fragment retention; malformed, nested, credentialed and unsafe targets fail closed while ordinary HTTP(S)/mailto behavior remains intact | Complete a shared renderer model, remaining stream/topic references, code/quote/list parity and accessibility semantics |
