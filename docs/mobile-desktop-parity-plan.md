...
| --- | --- | --- | --- |
| Realm discovery | Validate a Workspace server and load public settings | Partial: input is normalized to one credential-free HTTPS origin; the shared client applies bounded connect/request/socket timeouts; a successful public response is accepted only when its realm name, optional canonical realm, meeting URL and icon metadata are bounded and safe; timeout, network/TLS, 404, permission, throttle/5xx and malformed-response failures produce distinct content-free recovery messages before any pending organization is saved | Complete MockEngine status/body/redirect/TLS coverage and physical unreachable/certificate failure acceptance |
