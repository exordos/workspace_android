...
| Authentication | Login by nickname or email, password, six-digit OTP | Partial: an Activity-scoped login checkpoint now records only a bounded control-character-free nickname/email and a non-secret phase marker through `SavedStateHandle`, independently of the conditionally mounted login navigation graph; password, OTP, access/refresh tokens and project data remain memory-only. Activity recreation retains the live ViewModel, while process recreation from an in-flight request, OTP challenge or project picker fails closed to the credential screen with the safe login restored and an explicit re-entry message; success or explicit add-account cancellation removes all retained login metadata. A physical-device real-client MockEngine matrix now covers nickname and email bodies, exact token/project contracts, OTP challenge, rejected-code retry, successful project-scoped token persistence, invalid credentials, and recreated OTP/project steps without secret persistence | Complete the remaining malformed/missing/timeout/5xx and delayed-completion response matrix, backend acceptance with the primary account, then run intermediate-step process death physically |
