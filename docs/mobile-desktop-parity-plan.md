...
| External accounts | Connect, edit, reconnect, disconnect providers | Partial: strict Zulip DTO/request contracts, HTTPS-origin and identifier validation, write-only credential bodies, strong-revision updates, bounded owner-fenced pagination, lifecycle mutations, full-snapshot realtime projection and delete tombstones are implemented below the UI | Add the permission-aware account screen, conflict refresh, physical/backend acceptance and operation progress without ever retaining or redisplaying the API key |
| External chats | Select, deselect, move projections and show operations | Partial: strict account-scoped catalog/detail/assignment contracts, owner-fenced bounded pagination, revision-aware realtime projection and projection-stream cleanup are implemented below the UI | Add the functional selection/move surface, visible transition/error state, operations/retry support and physical/backend acceptance |
