...
Figma is strictly read-only for Android development: existing pages, frames,
components, and properties must never be created, edited, moved, or deleted.
When a required mobile state is absent, leave a comment, press Send, and verify
that the comment was actually posted.
...
- **Unavailable**: the backend contract does not support the capability. It is
  ...
  unless a specific temporary navigation state was explicitly approved by the
  product owner and is documented as incomplete.
...
| --- | --- | --- | --- |
| Realm discovery | Validate a Workspace server and load public settings | Complete for the current contract: input is normalized to one credential-free HTTPS origin; the shared client applies bounded connect/request/socket timeouts and rejects a final response whose scheme, host or effective port changed through redirects; a successful public response is accepted only when its realm name, optional canonical realm, meeting URL and icon metadata are bounded and safe; timeout, network/TLS, 404, permission, throttle/5xx and malformed/oversized-response failures produce distinct content-free recovery messages before any pending organization is saved; isolated MockEngine coverage exercises same/cross-origin redirects, the status matrix, malformed/oversized bodies and SSL/CIO/Android certificate failures through the real API client; physical DNS and expired-certificate acceptance passed on Android | Maintain the automated and physical fault matrix when the client or public settings contract changes |
| Authentication | Login by nickname or email, password, six-digit OTP | Partial: an Activity-scoped login checkpoint now records only a bounded control-character-free nickname/email and a non-secret phase marker through `SavedStateHandle`, independently of the conditionally mounted login navigation graph; password, OTP, access/refresh tokens and project data remain memory-only. Activity recreation retains the live ViewModel, while process recreation from an in-flight request, OTP challenge or project picker fails closed to the credential screen with the safe login restored and an explicit re-entry message; the phase marker remains consumable until the user meaningfully edits credentials, so repeated ViewModel creation cannot hide an interrupted-flow warning. Success or explicit add-account cancellation removes all retained login metadata. Physical-device real-client MockEngine coverage now exercises nickname and email bodies, exact token/project contracts, OTP challenge, rejected-code retry, successful project-scoped token persistence, invalid credentials, recreated OTP/project steps, and malformed/missing/timeout/503/delayed responses across credentials, OTP, project discovery and project-token confirmation. Live primary-account acceptance now passes nickname, OTP, project selection and authenticated messenger navigation. Real `kill -9` checks at the live OTP and project steps restore only the bounded login, clear every secret and show the explicit re-entry message. Retryable OTP/project checkpoints remain aligned with the visible step, typed Ktor timeouts produce an actionable message, and no fault creates a session or persists a secret | Complete live email-identity acceptance and maintain the physical recovery matrix when IAM or navigation contracts change |
| Session lifecycle | Refresh token, logout, expired-session recovery | Complete for the current contract: refresh is single-flight and owner-bound; timeout/network/429/5xx token-endpoint failures keep the account, receive two automatic retries, and then leave later triggers on the owner-scoped 1s/2s/4s/8s/16s/30s monotonic backoff outside the shared mutex. Only the explicit OAuth `invalid_grant` / backend invalid-refresh-token response removes the still-active rejected owner through the serialized cleanup path; generic refresh 401/403 and a protected-resource 401 after successful rotation preserve the session and return a recoverable error. Successful rotation resets that owner's schedule. The real client path is covered on Android for terminal invalid-grant, recoverable 401/403/429/503/network/malformed/identity-mismatch, same-call 503-to-success rotation, post-rotation 401 and four-call concurrency; a physical live endpoint request with a generated invalid refresh token removes only its isolated test owner. Explicit logout requires idempotent push cleanup, then removes owner-scoped state and the attachment cache before credentials, rolls the credential pair back on a storage failure, and removes the account registry entry last; injected Android cache/credential failures retain a retryable active owner. The account-scoped 6h/12h/24h/3d/7d/Never inactivity policy persists a bounded checkpoint across process death | Maintain the controlled and live endpoint matrix when IAM or client retry contracts change |
| Password recovery | Request reset, success state, replace password | Unavailable: the maintained desktop login has no live recovery action, and the current public IAM client is not authorized to send a reset code | Keep recovery controls hidden until a non-enumerating public-client reset contract and end-to-end permission are available |
...
| Main navigation | Feed, Messenger, Calendar, Mail and profile/settings | Partial: the signed-in shell now follows the maintained mobile Figma five-position order and 102 dp rounded panel, exposes semantic 54 dp tabs, restores each implemented top-level stack, and keeps message/deep-link entry owner-bound to Messenger; Calendar and Mail open intentional native "Уже скоро! В разработке!" states requested for the current product stage | Replace the temporary Calendar/Mail states when working product surfaces and approved mobile screens exist, request missing states through sent Figma comments, and complete process-death, multi-account, TalkBack and large-font acceptance; keep Services hidden until it has a working product destination |
...
| Folder management | Create, rename, delete, assign, pin, reorder/layout | Partial: current folder/folder-item endpoints back create, rename, delete, assign, remove, pin and unpin; the dedicated list/create flow follows mobile Figma frames `12169:30283` and `12169:30552`, creates a validated 1–64 character folder and assigns an ordered, de-duplicated chat selection with explicit partial-success reporting; selecting a displayed folder updates the shared catalog selection before returning to Chats; authoritative refresh follows every mutation | Add drag reorder/layout only when the desktop product contract exposes the same user action; if its mobile design is missing, leave a sent Figma comment and keep the design file read-only |
| Chat creation | Direct chat, group/private channel, public channel | Partial: direct chat and stream creation use separate full-screen mobile flows; stream creation matches the maintained name/member hierarchy, resolves the authoritative default topic and deliberately defaults desktop-only description/private/announcement fields that have no mobile design | Complete cross-client mutation E2E, permission revocation and retry/failure injection; request missing mobile settings through a sent Figma comment without changing the file |
...
| Topic management | Desktop-visible channel-wide stream plus create, rename, notification, read and done actions | Partial: the topic panel now exposes a read-only `Все темы` route backed by the real stream-filtered message endpoint, descending keyset pagination, exact-message navigation and stream-scoped realtime validation; all listed mutations use current endpoints and update the shared projection; backend-only delete/default/move controls remain hidden. The maintained mobile Figma section explicitly lacked this state, so its missing frame was requested in a verified comment without editing the design | Complete cold-offline persistence and cross-client/restart/accessibility E2E for the channel-wide stream; request product direction before exposing an action absent from desktop |
...
| Message sending | Compact Figma composer, text/Markdown source, user mentions, searchable Unicode emoji, Draft history, optimistic row, retry/remove failed send | Partial: the production composer now matches the approved mobile frame as one 48 dp base row ordered Attach → editor → Emoji → Draft history → Send/Save, with no desktop Write/Preview tabs or formatting-button strip; owner-scoped real users drive a bounded desktop-order `@` autocomplete that inserts escaped canonical UUID links; the pinned Unicode catalog inserts native multi-codepoint glyphs at the exact cursor/selection; Draft history opens the real single-top Drafts route; an encrypted account/conversation-scoped outbox is persisted before POST; optimistic rows survive Activity/process recreation and a fully cold offline exact-topic start; rejected and ambiguous outcomes expose different retry/verify/remove paths; successful responses are validated against the exact conversation and payload | Complete compact-row physical Figma comparison, multiline/IME/accessibility acceptance, emoji non-collapsed selection/2x-font/load-failure and mention account-switch/2x-font acceptance; expand real-backend/process-failure E2E to attachments, edits and concurrent sends; backend request idempotency is required before ambiguous delivery can be retried automatically without duplicate risk |
...
| Files and media | Upload, cancel, retry, download, preview, gallery | Partial: general MIME multi-upload, safe filenames, protected download/open, image preview and Android Share Sheet draft ingress are present; the selected-media strip matches the Figma 144 × 101 dp geometry and exposes conversation-scoped per-attachment upload progress; close cancels the active read/network job and retains the selection; every completed file identity is strictly validated and encrypted immediately so a process-restart retry skips it and resumes at the first incomplete file | Add true byte-range resumable transfer, richer video/document preview, camera/paste and gallery; complete controlled storage-failure and larger multi-file recovery acceptance |
...
| Typing/readers | Typing indicators and message reader details | Unavailable: the maintained desktop typing adapter is an explicit no-op, its readers adapter always returns an empty list, and the current backend exposes neither data path | Keep both controls hidden until real backend contracts exist; never simulate activity or readers |
...
| Settings | Theme, language, sound, sorting, folder layout, idle timeout | Partial: the Profile/Settings shell follows maintained mobile Figma frame `12052:41584` for the compact title, identity summary, organizations, description/settings sections and flat rows; saved organizations, current server, personal information, status, sound, auto sign-out, theme and the dedicated Figma folder list/create route remain functional. Account-scoped system/light/dark mode, six real notification sounds, standard/compact chat rows, unread priorities and inactivity presets are persisted and applied; unknown/corrupt values fail to independent safe defaults | Finish resource-backed language switching and any dedicated folder-presentation choices that receive an approved design; the current language remains explicitly read-only |
...
| Mail | Secure configured mail surface | Planned: the required tab currently opens a theme-aware native "Уже скоро! В разработке!" state with no fake mailbox or inert actions | Provide a working contract and approved mobile states, then implement message/compose/attachment/account-switch/session-expiry behavior; record the missing design through a sent Figma comment without editing the file |
| Calendar | Secure configured calendar surface | Planned: the required tab currently opens the matching native "Уже скоро! В разработке!" state with no fake events or inert actions | Provide a working contract and approved mobile states, then implement month/event/create/edit/account-switch/session-expiry behavior; record the missing design through a sent Figma comment without editing the file |
...
Folder display opens the dedicated mobile Figma list (`12169:30283`). Its
create route (`12169:30552`) uses the exact current folder and folder-item
contracts, validates the backend 64-character title limit, lets the user search
and select real chats, reports partial assignment without deleting a folder
that was already accepted, and shares the selected folder with the chat
catalog before returning. Dedicated orientation/system-folder choices remain
absent: the maintained desktop `showSystemFolders` flag currently persists but
does not alter its projection.
