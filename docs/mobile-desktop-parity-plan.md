...
| Organizations | Multiple saved organizations/accounts and switching | Partial: encrypted identity registry, add/return/logout UI, owner-bound request/refresh isolation, private attachment-cache partitioning, owner-scoped encrypted drafts/outbox and Room catalog/history snapshots, and account-aware link routing; a same-server two-project Pixel E2E passed warm switching, independent cold starts, catalog isolation, and cross-account message focus | Finish multi-account push routing, broaden the persisted entity set, then automate delayed-request and background-delivery fault cases |
...
| Chat catalog | Direct chats, channels, topics, unread counts, folders | Partial: bounded encrypted Room snapshots restore the active account's streams/topics before network startup, retain them through a cold offline launch, label them stale through the global recovery banner, and reconcile newer REST/realtime rows without rollback; physical cold-start, rotation and reconnect acceptance passed | Persist bindings/folders/folder items and unread aggregation metadata, then add controlled corrupt-row, account-switch and cursor-expiry E2E |
...
| Realtime | REST catch-up, websocket, epoch cursor reset | Partial: one retained repository owns a foreground-scoped socket; its generation/version cursor is Keystore-encrypted and account-scoped, restored before reconnect, advanced only after an event is applied, and cleared with that account on logout; every foreground/process recovery first drains bounded strict-generation REST pages and only then opens the socket; 20-second protocol pings close a half-open transport within 40 seconds, inbound frames are capped at 2 MiB, and flapping connections back off; REST `410` or socket `4410` clears expired server projections, preserves local outbox rows, and forces authoritative snapshot reload; the global banner reports delayed connecting/backoff and exposes a real foreground-only retry; streams/topics/latest conversation rows are now also durable and merge beneath newer live state | Persist the remaining server projections/sync metadata and complete controlled lifecycle/process-kill/410/4410/half-open E2E |
| Offline | Cache-first desktop state and outbox | Partial: encrypted drafts, realtime cursors and failed/ambiguous outgoing rows survive restart; an owner-scoped AES-256-GCM Room snapshot restores bounded streams, topics and latest server messages before realtime starts, allowing general catalog browsing and exact cached-topic deep links after a fully cold offline start; stale/retry UX is global and reconnection converges without replacing newer REST/realtime/outbox rows | Extend the same strict persistence contract to folders, bindings, users, feed/inbox projections and pagination markers; add a backend idempotency contract and controlled corruption/storage-pressure/long-outage E2E |
...
- Expand Room incrementally to bindings, folders/items, users, reactions,
  pagination cursors and sync metadata under the same bounds and owner fence.
- DataStore stores small non-secret preferences.
- Keystore-backed encryption also protects refresh/access credentials, push
  identity material, drafts and outbox state.
