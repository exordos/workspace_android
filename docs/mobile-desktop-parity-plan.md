...
| --- | --- | --- | --- |
| Realm discovery | Validate a Workspace server and load public settings | Partial: input is normalized to one credential-free HTTPS origin; the shared client applies bounded connect/request/socket timeouts and rejects a final response whose scheme, host or effective port changed through redirects; a successful public response is accepted only when its realm name, optional canonical realm, meeting URL and icon metadata are bounded and safe; timeout, network/TLS, 404, permission, throttle/5xx and malformed/oversized-response failures produce distinct content-free recovery messages before any pending organization is saved; isolated MockEngine coverage exercises same/cross-origin redirects, the status matrix, malformed/oversized bodies and an injected TLS handshake failure through the real API client | Complete physical certificate failure acceptance |
