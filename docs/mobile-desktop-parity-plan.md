...
| Message sending | Text, selection-aware Markdown formatting, user mentions, searchable Unicode emoji, local preview, optimistic row, retry/remove failed send | Partial: all ten maintained desktop formatting actions now mutate the exact selection/cursor with localized 48 dp controls and authoritative size-bound reconciliation; owner-scoped real users drive a bounded desktop-order `@` autocomplete that inserts escaped canonical UUID links, with physical query/Preview/send/edit/rotation acceptance; the real pinned Unicode catalog now backs a localized searchable composer picker that inserts native multi-codepoint glyphs at the exact cursor/selection and reconciles to the same authoritative draft bound, with physical alias-search/exact-cursor/Preview/send/edit/rotation/light/dark acceptance; Write/Preview renders the exact plain or ordered multi-reply outgoing body through the production message renderer without a server mutation; an encrypted account/conversation-scoped outbox is persisted before POST; optimistic rows survive Activity/process recreation and a fully cold offline exact-topic start; rejected and ambiguous outcomes expose different retry/verify/remove paths; successful responses are validated against the exact conversation and payload | Complete physical emoji non-collapsed selection/2x-font/load-failure and mention account-switch/2x-font acceptance, plus edit/existing-URN attachment preview, IME/composition and controlled process-death checks; expand real-backend/process-failure E2E to attachments, edits and concurrent sends; backend request idempotency is required before ambiguous delivery can be retried automatically without duplicate risk |
...
| Files and media | Upload, cancel, retry, download, preview, gallery | Partial: general MIME multi-upload, safe filenames, protected download/open, image preview and Android Share Sheet draft ingress are present; the selected-media strip now matches the Figma 144 × 101 dp geometry and exposes conversation-scoped per-attachment upload progress with cleanup on every terminal path | Add durable cancellation/resume, richer video/document preview, camera/paste and gallery |
