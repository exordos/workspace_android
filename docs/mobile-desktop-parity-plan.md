...
| Realtime | REST catch-up, websocket, epoch cursor reset | Partial: one retained repository owns a foreground-scoped socket; its generation/version cursor is Keystore-encrypted and account-scoped, restored before reconnect, advanced only after an event is applied, and cleared with that account on logout; every foreground/process recovery first drains bounded strict-generation REST pages and only then opens the socket; flapping connections back off, while REST `410` or socket `4410` clears expired server projections, preserves local outbox rows, and forces authoritative snapshot reload | Add a silent-socket epoch watchdog, visible connection health, durable entity snapshots, and controlled lifecycle/process-kill/410/4410 E2E |
