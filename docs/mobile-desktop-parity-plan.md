...
| Rich content | Markdown, mentions, Workspace references, links | Partial: canonical `urn:url:http(s)://...` links unwrap to the exact safe external target; canonical stream and scoped/unscoped topic URNs resolve unique active owner-bound routes, including authoritative direct-chat default topics; malformed, missing, duplicate, archived, cross-stream, nested, credentialed and unsafe targets fail closed while ordinary HTTP(S)/mailto behavior remains intact | Complete a shared renderer model plus code/quote/list parity and accessibility semantics; finish physical owner-switch/missing-catalog fault acceptance for conversation references |
