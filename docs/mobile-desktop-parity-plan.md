...
| Files and media | Upload, cancel, retry, download, preview, gallery | Partial: general MIME multi-upload, safe filenames, protected download/open, image preview and Android Share Sheet draft ingress are present; the selected-media strip matches the Figma 144 × 101 dp geometry and exposes conversation-scoped per-attachment upload progress; its existing close control now follows the desktop contract by cancelling the active read/network job, clearing progress on every terminal path and retaining the selection for a safe retry | Add durable resumable transfer, richer video/document preview, camera/paste and gallery |
