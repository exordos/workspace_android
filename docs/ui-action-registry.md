...
| MSG-A04 | Attachment picker | Opens Android multi-document picker and previews up to ten selected image/video/document/unknown-MIME URIs in the Figma-defined 144 × 101 dp strip; the exact attachment being uploaded has its own localized progress state | Cancellation is side-effect free; inaccessible URI and selection-limit errors remain visible; upload progress is cleared on success, failure, or cancellation | Files suite + exact Compose dimensions/progress instrumentation + physical-device selection |
| MSG-A05 | Remove or cancel selected attachment | The Figma-defined 20 dp close affordance inside a 48 dp accessible touch target removes an idle selection; on the actively uploading item it cancels the read/network job and retains that selection for retry | Cancellation clears progress on every terminal path and never creates a message; a later retry reuses only strictly validated, encrypted per-file upload checkpoints | Exact Compose dimensions/state test + FILE-004/005 |
...
| MSG-A07 | Send/save | Sends nonblank text, quote, and/or sequential file uploads using desktop-compatible `urn:image/video/file` metadata, or edits the selected message | Each successful file identity is encrypted before the next upload, so restart retries skip completed files; the outbox is persisted before message POST, while ambiguous message delivery still requires verification or explicit duplicate-risk confirmation | Send/outbox suite + FILE-004/005 + cross-client file E2E |
...
| MSG-A30 | Compact Figma message composer | Renders one 48 dp base row in the approved order: Attach, text editor, Emoji, Draft history and Send/Save. The row grows only for multiline input; attachments and reply/edit context remain outside it. No desktop Write/Preview tabs or formatting-button strip are rendered | Every visible icon is a real action with a localized description; unavailable actions expose a disabled state; the ViewModel remains authoritative for bounded editor text; the compact shell cannot cross conversation/account ownership | MSG-COMP-001–008 + exact-order/height/absence Compose instrumentation + physical Android 14 Figma comparison |
| MSG-A31 | Compact composer draft history | Opens the existing Drafts route directly from the history icon while preserving the exact current conversation draft and returning through normal Back navigation | Navigation is single-top; offline or sync failures remain represented by the existing Drafts/draft-sync recovery surfaces; the control is disabled until the conversation is ready | MSG-COMP-007/008 + navigation and physical Back acceptance |
| MSG-A32 | Composer `@` mention autocomplete | Detects the active desktop-compatible `@query`, filters the active owner-scoped real user catalog by canonical UUID/username/display name/email and replaces only that range with an escaped `[name](urn:user:<uuid>) ` link | Malformed/system/duplicate users and non-active query states expose no row; results are bounded to eight, every row is a localized 56 dp action, and the editor reconciles cursor/text to ViewModel acceptance without inventing a profile | MSG-COMP-017–022 + detection/filter/insertion unit tests + exact Compose instrumentation + physical Android 14 query/insert/send/edit/rotation/delete-cleanup acceptance; account-switch/2x-font pending |
| MSG-A33 | Composer emoji picker | Opens the complete bundled desktop-compatible Unicode catalog, searches glyphs and normalized shortcode aliases, and inserts the selected native glyph at the exact cursor or selected range | Catalog loading is off the UI thread; empty/failure/Retry/Close states are honest and functional; only bounded non-control catalog glyphs reach the ViewModel-authoritative draft reconciliation; unsupported sticker controls are absent instead of inert | MSG-COMP-023–028 + exact insertion/catalog unit tests + composer/reaction production-catalog Compose instrumentation + physical Android 14 `smile`/`thumbs up`, exact middle-cursor, send/edit/delete-cleanup, query-recreation and light/dark/rotation acceptance; non-collapsed physical selection/2x-font/load-failure pending |
