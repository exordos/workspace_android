...
| MSG-A19 | Rich message rendering and references | Renders body, links, quotes, ordered/unordered lists, inline/fenced code, metadata and deterministic author accents with theme-bound tokens; a bounded CommonMark structure model preserves list/quote/code meaning for TalkBack; ordinary safe links and canonical URL, user, message, stream and scoped/unscoped topic URNs open their exact external or owner-bound mobile destination | Theme changes rebuild the renderer; all normal-size text pairs meet WCAG AA 4.5:1; unsafe URL URNs never receive Workspace credentials, while malformed, missing, archived, duplicate, cross-scope, response-mismatched or account-raced Workspace references fail closed with a visible error instead of guessing a destination | MSG-RICH-001–010 + structure/contrast unit tests + physical-device light/dark/reference navigation |
