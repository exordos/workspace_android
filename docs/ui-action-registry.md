...
| PLAT-A13 | Owner-scoped cold-offline Room snapshot | Before realtime startup, decrypts and hydrates bounded streams, topics and latest server messages for only the active credential owner; cached exact-topic links and normal catalog navigation remain usable with one global stale banner | AES-GCM associated data rejects moved/replayed/cross-scope rows; invalid/oversized rows are skipped individually; newer REST/realtime/local rows win; logout clears only that owner and no `local-*` outbox row is duplicated | OFFLINE-001–012 + exact store instrumentation + physical Pixel |
