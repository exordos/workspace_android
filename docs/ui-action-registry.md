...
| MSG-A26 | First-unread marker and viewport read | Latest history and the strict earliest incoming-unread lookup run concurrently; an off-page unread loads a bidirectional context window and anchors a non-interactive marker; only a real list drag followed by at-least-half visibility coalesces rows into one canonical `read_up_to` boundary | Exact routes retain priority; malformed/cross-chat/read/own candidates fail closed; no eager read on load/focus; confirmation is exact-scope/read-true; realtime full/batch events are idempotent and stale pages cannot restore unread; failures preserve unread state and surface an error | MSG-UNREAD-001–011; focused automated 003–007/010, physical sandbox and remaining fault cases pending |
