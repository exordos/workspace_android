...
| MSG-A23 | Load previous/next history and exact-message context | Normal entry loads the bounded latest page; an exact route first renders its canonical detail anchor, atomically replaces stale projection rows with strict older/anchor/newer context, and exposes automatic plus explicit continuation in both directions | Local outbox and newer overlapping realtime edits survive replacement; one failed side remains retryable, rejected/malformed markers reset safely, account switches discard late results, and cross-chat/wrong-side/out-of-order rows fail closed | MSG-HIST-001–016 + physical Pixel 55-message bidirectional/rotation acceptance |
...
| PLAT-A13 | Owner-scoped cold-offline Room snapshot | Before realtime startup, migrates schema-v1 history in place, then decrypts and hydrates bounded streams, topics, latest server messages, full folders/items, users and stream bindings for only the active credential owner; independent Feed/Starred projections and the Inbox success marker preserve exact content-ready state | AES-GCM associated data rejects moved/replayed/cross-scope rows; invalid relationships/enums/oversized rows are skipped individually; newer or authoritatively empty REST/realtime/local projections win; logout clears only that owner and no `local-*` outbox row is duplicated | OFFLINE-001–016 + exact store/migration instrumentation + physical Pixel |
