...
| PLAT-A13 | Owner-scoped cold-offline Room snapshot | Before realtime startup, migrates schema-v1 history in place, then decrypts and hydrates bounded streams, topics, latest server messages, full folders/items, users and stream bindings for only the active credential owner; cached exact-topic, catalog and member views remain usable with one global stale banner | AES-GCM associated data rejects moved/replayed/cross-scope rows; invalid relationships/enums/oversized rows are skipped individually; newer or authoritatively empty REST/realtime/local projections win; logout clears only that owner and no `local-*` outbox row is duplicated | OFFLINE-001–013 + exact store/migration instrumentation + physical Pixel |
