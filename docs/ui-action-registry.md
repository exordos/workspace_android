...
| MSG-A05 | Remove selected attachment | Clears only the selected pending attachment through the Figma-defined 20 dp visual close affordance inside a 48 dp accessible touch target | The action is disabled while that attachment is uploading; no external failure | Exact Compose dimensions/state test |
