...
| MSG-A19 | Rich message rendering and references | Renders body, links, quotes, inline code, metadata and deterministic author accents with theme-bound tokens; ordinary safe links and canonical `urn:url:http(s)://...` references open through a browsable platform intent, while canonical `urn:stream:<uuid>` and `urn:topic:<uuid>`/`urn:topic:<stream-uuid>:<topic-uuid>` references resolve an exact owner-bound mobile route | Theme changes rebuild the renderer; all normal-size text pairs meet WCAG AA 4.5:1; malformed/unsafe URL URNs never receive Workspace credentials, and missing, archived, duplicate, cross-stream or account-raced conversation references fail closed with a visible error instead of guessing a destination | MSG-RICH-001–006 + token contrast unit test + physical-device light/dark/reference navigation |
