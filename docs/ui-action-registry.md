...
| MSG-A19 | Rich message rendering and external links | Renders body, links, quotes, inline code, metadata, and deterministic author accents with theme-bound tokens; ordinary safe links and canonical `urn:url:http(s)://...` references open through a browsable platform intent | Theme changes rebuild the renderer; all normal-size text pairs meet WCAG AA 4.5:1; malformed, nested, credentialed and unsafe URL URNs fail closed with a visible error and never receive Workspace credentials | MSG-RICH-001–003 + token contrast unit test + physical-device light/dark check |
